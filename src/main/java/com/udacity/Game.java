...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        String res1 = checkHorisonatlLines();
        String res2 = checkVerticlaLines();
        String res3 = checkMainDiagonalLines();
        String res4 = checkSecondaryDiagonalLines();

        String[] resultsOfChecks = {res1, res2, res3, res4};

        for (String res: resultsOfChecks
             ) {
            if (!("None".equals(res))) {
                return res;
            }
    ...
    /**
     * Check if there is one horizontal line that all maked by one gamer
     * If no one win return "None"
     * If x player win return "X wins"
     * If o player win return "O wins"
     * @return String indicating the outcome of the game: "X wins" or "O wins" or "None"
     */
    public String checkHorisonatlLines() {

        for (int i = 0; i < 3; i++) {

            List<Character> unique = new String(grid[i])
                    .chars()
                    .mapToObj(el -> (char) el)
                    .distinct()
                    .collect(Collectors.toList());

            long numberEls = unique.size();
            char element = unique.get(0);

            if (numberEls == 1 && element != '-') {
                return element == 'x' ? "X wins" : "O wins";
            }
        }
    ...
    /**
     * Check if there is one vertical line that all maked by one gamer
     * If no one win return "None"
     * If x player win return "X wins"
     * If o player win return "O wins"
     * @return String indicating the outcome of the game: "X wins" or "O wins" or "None"
     */
    public String checkVerticlaLines() {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            for (int i = 0; i < 3; i++) {
                array.add(gridAt(i, j));
            }

            List<Character> unique = array
                    .stream()
                    .distinct()
                    .collect(Collectors.toList());
    ...
    /**
     * Check if all elements in main diagonal belongs to one gamer
     * If no one win return "None"
     * If x player win return "X wins"
     * If o player win return "O wins"
     * @return String indicating the outcome of the game: "X wins" or "O wins" or "None"
     */
    public String checkMainDiagonalLines() {

        ArrayList<Character> array = new ArrayList<Character>();

        for (int i = 0; i < 3; i++) {
            array.add(gridAt(i,i));
        }
    ...
    /**
     * Check if all elements in secondary diagonal belongs to one gamer
     * If no one win return "None"
     * If x player win return "X wins"
     * If o player win return "O wins"
     * @return String indicating the outcome of the game: "X wins" or "O wins" or "None"
     */
    public String checkSecondaryDiagonalLines() {

        ArrayList<Character> array = new ArrayList<Character>();
        array.add(gridAt(0,2));
        array.add(gridAt(1,1));
        array.add(gridAt(2, 0));
